...
class Client:
    ...
    def __init__(self, args, client_idx, train_data_loader, test_data_loader):
        ...
        self.scheduler = MinCapableStepLR(self.args.get_logger(), 
            self.optimizer,
    ...
    def load_default_model(self):
        ...
        default_model_path = os.path.join(self.args.
            get_default_model_folder_path(), model_class.__name__ + ".model")
    ...
    def load_model_from_file(self, model_file_path):
        ...
        if os.path.exists(model_file_path):
            try:
                ...
                warning("Couldn't load model."
                 "Attempting to map CUDA tensors to CPU to solve error.")

                model.load_state_dict(torch.load(model_file_path, 
                    map_location=torch.device('cpu')))
        else:
            self.args.get_logger().warning("Could not find model: {}".
    ...
    def save_model(self, epoch, suffix):
        ...
        """
        self.args.get_logger().
        debug("Saving model to flat file storage. Save #{}", epoch)
        ...
        full_save_path = os.path.join(self.args.get_save_model_folder_path(), 
            "model_" + str(self.client_idx) + "_" + str(epoch) + "_" + 
            suffix + ".model")
    ...
    def calculate_class_precision(self, confusion_mat):
        ...
        """
        return numpy.diagonal(confusion_mat) / numpy.
        sum(confusion_mat, axis=0)
    ...
    def test(self):
        ...
        self.args.get_logger().debug('Test set: Accuracy: {}/{} ({:.0f}%)'.
        ...
        self.args.get_logger().debug("Confusion Matrix:\n" + 
            str(confusion_mat))
        self.args.get_logger().debug("Class precision: {}".
            format(str(class_precision)))
        self.args.get_logger().debug("Class recall: {}".
            format(str(class_recall)))
