...
def convert_distributed_data_into_numpy(distributed_dataset):
    """
    Converts a distributed dataset (returned by a data distribution method)
     from Tensors into numpy arrays.
    ...
    for worker_idx in range(len(distributed_dataset)):
        ...
        X_ = numpy.array([tensor.numpy() for batch in worker_training_data 
            for tensor in batch[0]])
        Y_ = numpy.array([tensor.numpy() for batch in worker_training_data 
