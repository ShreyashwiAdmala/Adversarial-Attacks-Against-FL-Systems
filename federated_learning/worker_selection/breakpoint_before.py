...
class BeforeBreakpoint(SelectionStrategy):
    """
    Will not select poisoned workers before the break point epoch, 
    but will select the poisoned workers at and after the break point epoch.
    """
    ...
    def select_round_workers(self, workers, poisoned_workers, kwargs):
        ...
        if current_epoch_number >= breakpoint_epoch:
            ...
            selected_workers = random.sample(non_poisoned_workers,
             num_workers)
