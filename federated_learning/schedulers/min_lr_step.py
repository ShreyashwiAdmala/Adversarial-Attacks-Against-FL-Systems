class MinCapableStepLR:
    ...
    def __init__(self, logger, optimizer, step_size,
     gamma, min_lr):
        """
        :param logger: logger
    ...
    def update_lr(self):
        if self.optimizer.param_groups[0]['lr'] * 
        self.gamma >= self.min_lr:
            self.optimizer.param_groups[0]['lr'] *= self.gamma
        else:
            self.logger.
            warning("Updating LR would place "
                "it below min LR. Skipping LR update.")

        self.logger.debug("New LR: {}".format(self.optimizer.
            param_groups[0]['lr']))
